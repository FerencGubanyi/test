...
Normalisation matches the weekend run that produced R²=+0.9965:
  std = full N×N matrix std (d.std())  — NOT row_sum std
  row_std stored separately so evaluate() can denormalise to passenger scale.
"""
...
def _load_od_excel(path: str) -> tuple:
    ...
    # Row 1 = old format (2019-2023), row 3/4 = new format (2024+)
    stations = []; data_start = 2
...
def build_bart_graph(gtfs_dir, stations):
    ...
    snm = dict(zip(stops.stop_id, stops.stop_name))
    s2i = {s: i for i, s in enumerate(stations)}
    t2r = dict(zip(trips.trip_id, trips.route_id))
    edges = {}; hes = {}; ct = None; pi = None
    for _, row in st.sort_values(["trip_id", "stop_sequence"]).iterrows():
        tid = row.trip_id; sn = snm.get(row.stop_id, "")
        m = _fuzzy_match(sn, stations); ni = s2i.get(m)
        rid = t2r.get(tid, "unknown")
    ...
    el = list(edges.keys())
    ei = torch.tensor([[e[0] for e in el] + [e[1] for e in el],
                        [e[1] for e in el] + [e[0] for e in el]], dtype=torch.long)
    ...
    return {"edge_index": ei, "edge_attr": ea, "station_to_idx": s2i,
            "hyperedge_index": (torch.tensor([hs, hr], dtype=torch.long)
                                if hs else torch.zeros(2, 0, dtype=torch.long)),
...
def generate_bart_synthetic_scenarios(baseline_matrices, n_synthetic=60, rng_seed=42):
    ...
    for _ in range(n_synthetic):
        ...
        aff = rng.choice(N, size=na, replace=False)
        delta = np.zeros_like(base); mag = rng.uniform(0.05, 0.25)
        for s in aff:
            id_ = base[:, s] * mag * rng.uniform(0.5, 1.5)
            na_ = np.setdiff1d(np.arange(N), aff)
...
def load_bart_transfer_dataset(data_dir=DATA_DIR, n_synthetic=60, verbose=True):
    ...
    def _pack(d, bf, name, split, is_real):
        # std: full N×N matrix std — matches weekend training normalisation
        std     = float(d.std()) + 1e-8
        # row_std: used by evaluate() to convert predictions back to passenger scale
        row_std = float(d.sum(axis=1).std()) + 1e-8
        scenarios.append({
            "node_features":       compute_bart_node_features(bf),
            "delta_od":            d,
            "delta_od_normalized": d / std,
            "std":                 std,
            "row_std":             row_std,
            "is_real":             is_real,
            "label":               name,
    ...
    def _real(bl, al, name, split):
        ...
        d = af - bf
        _pack(d, bf, name, split, is_real=True)
        if verbose: print(f"  Real '{name}': MAE={np.abs(d).mean():.2f}, split={split}")

    # berryessa = val (thesis reference scenario)
    # antioch   = train (clean post-COVID signal, helps the model generalise)
    _real("before_berryessa", "after_berryessa", "berryessa_extension_2020", split="val")
    _real("before_antioch",   "after_antioch",   "antioch_extension_2023",   split="train")

    bl_mats = []
    for lbl in ["baseline_2019_01", "baseline_2019_06", "baseline_2019_10", "before_berryessa"]:
        if lbl in od:
            bs, bm = od[lbl]
            _, ba, _ = _align_matrices(bs, bm, all_st, np.zeros((len(all_st), len(all_st))))
            bl_mats.append(ba)
    ...
    if bl_mats:
        ...
        for i, d in enumerate(generate_bart_synthetic_scenarios(bl_mats, n_synthetic)):
            _pack(d, ref, f"synthetic_bart_{i:03d}", split="train", is_real=False)

    tc = sum(1 for s in scenarios if s["split"] == "train")
    vc = sum(1 for s in scenarios if s["split"] == "val")
