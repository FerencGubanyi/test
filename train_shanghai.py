"""
train_shanghai.py — Train GAT+LSTM and Hypergraph+LSTM on Shanghai MetroFlow.

Usage:
    python train_shanghai.py --model gat        --output checkpoints/shanghai_gat.pt
    python train_shanghai.py --model hypergraph --output checkpoints/shanghai_hg.pt

Key difference from BART:
  - 302 stations (vs 51) — closer to BKK's 1419 zones
  - 16 real ΔOD scenarios from consecutive weekly differences
  - Sparse OD matrix — uses row-sum target (outgoing flow change per station)
  - Config must match 302 nodes and 16-dim features
"""
...
# ---------------------------------------------------------------------------
# Config subclasses for Shanghai (302 stations, 16 features)
# ---------------------------------------------------------------------------

class SHGATConfig(GATConfig):
    NUM_ZONES        = 302
    GAT_IN_CHANNELS  = 16
    OUTPUT_SIZE      = 302
...
class SHHGConfig(HypergraphConfig):
    NUM_ZONES        = 302
    HG_IN_CHANNELS   = 16
    OUTPUT_SIZE      = 302
    DEVICE           = "cuda" if torch.cuda.is_available() else "cpu"
...
def scenario_to_inputs(graph_data, scenario, device):
    x             = torch.tensor(scenario["node_features"],
                                 dtype=torch.float, device=device)
    x_seq         = [x]
    edge_index    = graph_data["edge_index"].to(device)
    # (1, 8) to match lstm_out shape (1, hidden)
    scenario_feat = torch.zeros(8, dtype=torch.float, device=device).unsqueeze(0)

    delta_od  = scenario["delta_od"]
    target_np = delta_od.sum(axis=1)          # per-station outgoing ΔOD
...
def build_incidence_matrix(graph_data, n_nodes, device):
    ...
    H[he_index[0].clamp(0, n_nodes-1),
      he_index[1].clamp(0, n_edges-1)] = 1.0
    return H
...
def train_one_epoch(model, model_type, optimizer,
    ...
    for idx in np.random.permutation(len(train_scenarios)):
        s = train_scenarios[int(idx)]
        x_seq, edge_index, sf, target, _ = scenario_to_inputs(
            graph_data, s, device
        )
        weight = real_weight if s["is_real"] else 1.0
        optimizer.zero_grad()
        pred = model(x_seq, edge_index, sf) if model_type == "gat" \
               else model(x_seq, sf)
...
def evaluate(model, model_type, val_scenarios, graph_data, device):
    ...
    for s in val_scenarios:
        ...
        all_targets.append(s["delta_od"].sum(axis=1))
...
def main():
    ...
    parser.add_argument("--output",      type=str,   default="checkpoints/shanghai_pretrained.pt")
    parser.add_argument("--base_dir",    type=str,   default="/content/data/shanghai/MetroFlow")
    ...
    H = None
    if args.model == "gat":
        model = GATLSTMModel(cfg=SHGATConfig, scenario_feat_dim=8).to(device)
    else:
        model = HypergraphLSTMModel(cfg=SHHGConfig, scenario_feat_dim=8).to(device)
    ...
    if val_scenarios and os.path.exists(args.output):
        ...
        print(f"  MAE:  {f['mae']:.3f} passengers/station")
        print(f"  RMSE: {f['rmse']:.3f}")
        print(f"  R²:   {f['r2']:+.4f}")
        print("\nReady for transfer.py → BKK fine-tuning.")
