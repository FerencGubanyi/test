...
--------------------------------------------------
METR-LA architecture validation, fully compatible with the actual
GATLSTMModel and HypergraphLSTMModel from the repo.

Key interface facts from reading the real model code:
  GATLSTMModel.forward(x_seq: List[Tensor], edge_index, scenario_feat) -> (1, N)
  HypergraphLSTMModel.forward(x_seq: List[Tensor], scenario_feat)      -> (1, N)
...
def main():
    ...
    if r > 0.85:
        ...
        print("    METR-LA R² > 0.55 és BKK ~ 0 → dataset problem.")
    else:
        print("\n✗   Low R² METR-LA")
